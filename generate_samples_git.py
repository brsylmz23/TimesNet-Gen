...
def load_model(checkpoint_path, args):
    ...
    # Import from models/ directory (now a proper Python package with __init__.py)
    try:
        ...
        try:
            ...
            raise ImportError(
                "Could not import TimesNetPointCloud. "
                "Please ensure TimesNet_PointCloud.py is in models/ directory or root directory."
            )
...
def main():
    ...
    parser.add_argument('--checkpoint', type=str, 
                        default='./TimesNet Pointcloud Phase 1.pth',
                        help='Path to pre-trained model checkpoint')
    parser.add_argument('--latent_bank', type=str, 
                        default='./TimesNetGen Latent Bank Phase 1.npz',
    ...
    try:
        ...
        # Use data directory for real (recon) data, gen_dir for generated data
        ...
        sys.argv = [
            ...
            '--recon_dir', data_dir,  # Use data directory for real data
            '--out', os.path.join(args_cli.output_dir, 'hvsr_analysis'),
            '--vae_dir', '',    # Explicitly empty to disable VAE
            '--vae_gen_dir', '',  # Explicitly empty to disable VAE
        ]
