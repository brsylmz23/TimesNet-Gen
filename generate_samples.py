...
def generate_samples_from_latent_bank(model, latent_bank_path, station_id, num_samples, args, encoder_std=None):
    ...
    with torch.no_grad():
        for i in range(num_samples):
            ...
            # Add Gaussian noise if encoder_std is provided
            ...
            # Use first selected sample's means/stdev for decoding
            means_b = torch.from_numpy(means[selected_indices[0]]).float().unsqueeze(0)  # (1, seq_len, d_model)
            stdev_b = torch.from_numpy(stdevs[selected_indices[0]]).float().unsqueeze(0)  # (1, seq_len, d_model)
            ...
            generated = model.project_features_for_reconstruction(mixed_features_torch, means_b, stdev_b)
            
            # Store - transpose to (3, 6000)
            generated_np = generated.squeeze(0).cpu().numpy().T  # (6000, 3) → (3, 6000)
...
def _compute_hvsr_simple(signal: np.ndarray, fs: float = 100.0):
    ...
    try:
        ...
        # Simple preprocessing: linear detrend
        def detrend(x):
            n = len(x)
        ...
        sm_h = smooth(combined_h, window=5)
        sm_v = smooth(amp_ud, window=5)
...
def main():
    ...
    # Try to load encoder_std from Phase 0
    ...
    if os.path.exists(encoder_std_path):
        ...
        print(f"[INFO] Will add Gaussian noise to latent space for better diversity")
    else:
        print(f"[INFO] No encoder_std found, using bootstrap without noise injection")
