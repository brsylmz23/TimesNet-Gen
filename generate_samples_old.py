...
Examples:
  ...
  python generate_samples.py --stations 0205 1716 --num_samples 50
    ...
    for station_id in args_cli.stations:
        ...
        pattern = os.path.join(args.root_path, f"*{station_id}*.mat")
        ...
        if len(station_files) == 0:
            ...
            all_files.extend([(f, station_id) for f in station_files])
    
    if len(all_files) == 0:
        raise FileNotFoundError(f"No data files found in {args.root_path} for stations {args_cli.stations}")
    
    print(f"[INFO] Total files loaded: {len(all_files)} samples from {len(args_cli.stations)} stations")
